...
export interface AutosaveControllerOptions { debounceMs?: number; maxRetries?: number; retryDelayMs?: number; save: (request: AutosaveRequest) => Promise<Workflow>; onStatusChange?: (status: SaveStatus) => void; onSaved?: (request: AutosaveRequest, saved: Workflow, isLatest: boolean) => void; onFailed?: (request: AutosaveRequest, error: unknown) => void; getLatestRevision: () => number; }
...
export class AutosaveController {
  ...
  constructor(options: AutosaveControllerOptions) { this.options = { debounceMs: AUTOSAVE_DEBOUNCE_MS, maxRetries: AUTOSAVE_MAX_RETRIES, retryDelayMs: AUTOSAVE_RETRY_DELAY_MS, ...options }; }
