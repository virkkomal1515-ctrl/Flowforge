...
export function WorkflowList() {
  ...
  const create = useMutation({ mutationFn: createWorkflow, onSuccess: (workflow) => { queryClient.setQueryData(workflowQueryKeys.detail(workflow.id), workflow); queryClient.invalidateQueries({ queryKey: workflowQueryKeys.list() }); toast({ tone: "success", title: "Workflow created", message: "Opening the new draft." }); router.push(`/workflows/${encodeURIComponent(workflow.id)}`); }, onError: (error) => toast({ tone: "error", title: "Could not create workflow", message: error instanceof Error ? error.message : "Please try again." }) });
  const remove = useMutation({ mutationFn: deleteWorkflow, onSuccess: (_, workflowId) => { queryClient.removeQueries({ queryKey: workflowQueryKeys.detail(workflowId) }); queryClient.invalidateQueries({ queryKey: workflowQueryKeys.list() }); setWorkflowToDelete(null); toast({ tone: "success", title: "Workflow deleted" }); }, onError: (error) => toast({ tone: "error", title: "Could not delete workflow", message: error instanceof Error ? error.message : "Please try again." }) });

  const filteredWorkflows = useMemo(() => {
    if (!workflows.data) return [];
    const query = search.trim().toLowerCase();
    return workflows.data.filter((workflow) => (!query || workflow.name.toLowerCase().includes(query) || workflow.description.toLowerCase().includes(query)) && (status === "all" || workflow.status === status));
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-950">Your workflows</h2>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <label className="relative min-w-0 flex-1"><span className="sr-only">Search workflows</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search workflows…" className="h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50" /></label>
        <label className="sm:w-40"><span className="sr-only">Filter by status</span><select value={status} onChange={(event) => setStatus(event.target.value as "all" | Workflow["status"])} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50"><option value="all">All statuses</option><option value="draft">Draft</option><option value="published">Published</option></select></label>
      </div>
