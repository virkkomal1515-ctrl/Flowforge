...
function WorkflowCanvasContainer({ workflowId }: { workflowId: string }) {
  const detail = useQuery({ queryKey: workflowQueryKeys.detail(workflowId), queryFn: () => fetchWorkflow(workflowId), retry: false });

  if (detail.isPending) return <div role="status" className="rounded-2xl border border-slate-200 bg-white p-8 text-sm text-slate-600">Loading workflow…</div>;
  if (detail.isError) return <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-8 text-sm text-red-700">{detail.error.message}</div>;
  return <CanvasEditor key={detail.data.id} initialWorkflow={detail.data} persisted />;
}

export function WorkflowCanvas({ workflowId }: { workflowId?: string }) {
  return <ReactFlowProvider>{workflowId ? <WorkflowCanvasContainer workflowId={workflowId} /> : <CanvasEditor initialWorkflow={sampleWorkflow} persisted={false} />}</ReactFlowProvider>;
}
