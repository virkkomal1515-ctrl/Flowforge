...
function CanvasEditor() {
  ...
  const applyConfig = (nodeId: string, config: NodeConfig) => {
    const next = updateNodeConfig(workflow, nodeId, config);
    if (!next) { setMessage("Configuration rejected by the workflow domain."); return; }
    setWorkflow(next); setMessage("Node configuration updated.");
  };
