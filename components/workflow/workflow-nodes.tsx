...
function Frame({ node, children }: { node: CanvasNode; children: React.ReactNode }) { return <div role="group" aria-label={`${labels[node.data.nodeType]}${node.data.hasValidationError ? ", validation error" : ""}`} data-validation-error={node.data.hasValidationError || undefined} className={`relative min-h-24 w-52 rounded-xl border-2 px-4 py-3 shadow-sm ${tone[node.data.nodeType]} ${node.data.hasValidationError ? "border-red-600 bg-red-50 ring-2 ring-red-200" : ""}`}>
