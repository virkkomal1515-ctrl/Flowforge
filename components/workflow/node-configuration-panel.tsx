...
type FormValuesByType = {
  trigger: { triggerName: string; triggerType: "manual" };
  action: { actionName: string; operation: "assign" | "update_status" | "create_record"; parameter?: string };
  condition: { field: string; operator: "equals" | "not_equals" | "greater_than" | "less_than"; comparisonValue: string };
  notification: { recipient: string; message: string };
  end: { completionLabel?: string };
};
type FormValues = FormValuesByType[WorkflowNode["type"]];
...
function initialValues<T extends WorkflowNode>(node: T): FormValuesByType[T["type"]] { return node.config as FormValuesByType[T["type"]]; }
...
export function NodeConfigurationPanel({ node, onApply }: Props) {
  ...
  const submit = (values: FormValues) => {
    ...
    if (!parsed.success) { parsed.error.issues.forEach((issue) => { const field = issue.path[0]; if (typeof field === "string") setError(field as keyof FormValues, { type: "zod", message: issue.message }); }); return; }
  ...
  const fieldRegister = register as UseFormRegister<FormValues>;
  return <aside className="border-t border-slate-200 bg-white p-4 lg:border-l lg:border-t-0" aria-label="Node properties">
    ...
    <form onSubmit={handleSubmit(submit)} noValidate className="space-y-4">
      {fields[node.type].map((field) => {
        const error = errors[field.name]?.message; const id = `node-${node.id}-${field.name}`;
        ...
        return <div key={field.name}><label htmlFor={id} className="block text-sm font-medium text-slate-800">{field.label}{field.required ? <span aria-hidden="true"> *</span> : null}</label>{options ? <select id={id} required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} {...fieldRegister(field.name as keyof FormValues)} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus-visible:border-slate-900 focus-visible:ring-2 focus-visible:ring-slate-300">{options.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : field.name === "message" ? <textarea id={id} rows={4} required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} {...fieldRegister(field.name as keyof FormValues)} className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none focus-visible:border-slate-900 focus-visible:ring-2 focus-visible:ring-slate-300" /> : <input id={id} type={field.type ?? "text"} required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} {...fieldRegister(field.name as keyof FormValues)} className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none focus-visible:border-slate-900 focus-visible:ring-2 focus-visible:ring-slate-300" />}{error ? <p id={`${id}-error`} role="alert" className="mt-1 text-sm text-red-700">{String(error)}</p> : null}</div>;
      })}
