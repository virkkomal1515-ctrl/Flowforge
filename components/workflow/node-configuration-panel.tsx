...
import { nodeConfigSchemas, validateNodeConfigInput } from "@/domain/workflow/forms";

type FormValues = Record<string, string>;

type Props = { node: WorkflowNode | undefined; onApply: (nodeId: string, config: NodeConfig) => void };
...
export function NodeConfigurationPanel({ node, onApply }: Props) {
  ...
  const submit = (values: FormValues) => {
    ...
    if (!parsed.success) {
      parsed.error.issues.forEach((issue) => { const field = issue.path[0]; if (typeof field === "string") setError(field, { type: "zod", message: issue.message }); });
      return;
    }
    const config = validateNodeConfigInput(node.type, parsed.data);
    if (!config) { setError("root", { type: "domain", message: "This configuration is not valid for the selected node." }); return; }
  ...
  return <aside className="border-t border-slate-200 bg-white p-4 lg:border-l lg:border-t-0" aria-label="Node properties">
    ...
    <form onSubmit={handleSubmit(submit)} noValidate className="space-y-4">
      {fields[node.type].map((field) => {
        ...
        return <div key={field.name}><label htmlFor={id} className="block text-sm font-medium text-slate-800">{field.label}{field.required ? <span aria-hidden="true"> *</span> : null}</label>{options ? <select id={id} aria-required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} {...register(field.name)} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus-visible:border-slate-900 focus-visible:ring-2 focus-visible:ring-slate-300">{options.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : field.name === "message" ? <textarea id={id} rows={4} aria-required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} {...register(field.name)} className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none focus-visible:border-slate-900 focus-visible:ring-2 focus-visible:ring-slate-300" /> : <input id={id} type={field.type ?? "text"} aria-required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} {...register(field.name)} className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none focus-visible:border-slate-900 focus-visible:ring-2 focus-visible:ring-slate-300" />}{error ? <p id={`${id}-error`} role="alert" className="mt-1 text-sm text-red-700">{String(error)}</p> : null}</div>;
      })}
